export default {
  //基础sdk
  base: {
    url: '//static.vhallyun.com/jssdk/vhall-jssdk-base/vhall-msg-1.1.1.js'
  },
  //播放器sdk
  player: {
    name: 'VhallPlayer',
    url: '//static.vhallyun.com/jssdk/vhall-jssdk-player/latest/vhall-jssdk-player-2.4.3.js'
  },
  //聊天sdk
  chat: {
    name: 'VhallChat',
    url: '//static.vhallyun.com/jssdk/vhall-jssdk-chat/2.1.5/vhall-jssdk-chat-2.1.5.js'
  },
  //互动sdk
  interaction: {
    name: 'VhallRTC',
    url: '//static.vhallyun.com/jssdk/vhall-jssdk-interaction/latest/vhall-jssdk-interaction-2.3.8.js'
  },
  //文档sdk
  doc: {
    name: 'VHDocSDK',
    url: '//static.vhallyun.com/jssdk/vhall-jssdk-doc/latest/vhall-jssdk-doc-3.2.2.js'
  },
  questionnaire: {
    name: 'VhallQuestionnaire',
    url: '//cnstatic01.e.vhall.com/common-static/middle/questionnaire/1.0.3/questionnaire_service.js'
  },
  // 数据上报sdk
  report: {
    name: 'VhallReport',
    url: '//cnstatic01.e.vhall.com/common-static/middle/middle-log/1.0.4/index.min.js'
  }
};
